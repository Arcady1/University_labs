...
void ReadXMLData() {
  ...
  if (!doc.LoadFile("../..\\Lesson116\\source\\settings.xml")) {
    cout << "error" << endl;
  ...
  const char* textNode = element->FirstChild("graphPointsCountX")->FirstChild()->Value();

  str = textNode;
  graphPointsCountX = stoi(str, nullptr, 10);
  
  str = element->FirstChild("graphPointsCountY")->FirstChild()->Value();
  graphPointsCountY = stoi(str, nullptr, 10);
  
  str = element->FirstChild("objectsCount")->FirstChild()->Value();
  objectsCount = stoi(str, nullptr, 10);
  
  str = element->FirstChild("offsetX")->FirstChild()->Value();
  offsetX = stof(str, nullptr);
  
  str = element->FirstChild("offsetY")->FirstChild()->Value();
  offsetY = stof(str, nullptr);
  
  str = element->FirstChild("offsetZ")->FirstChild()->Value();
  offsetZ = stof(str, nullptr);
  
  str = element->FirstChild("objectVelocity")->FirstChild()->Value();
  objectVelocity = stof(str, nullptr);
  
  cout << graphPointsCountX << ' ' << graphPointsCountY << ' ' << objectsCount << endl;
}
...
public:
	...
	void move() {
		if (!bpath.empty()) {
			...
			float lengh = sqrt((p2.x - p1.x)*(p2.x - p1.x) + (p2.y - p1.y)*(p2.y - p1.y) + (p2.z - p1.z)*(p2.z - p1.z));
			...
			bool yb = p2.y > p1.y;
			bool zb = p2.z > p1.z;
			if (xb && (lastPos.x > p2.x || lastPos.x < p1.x) || !xb && (lastPos.x > p1.x || lastPos.x < p2.x)
				&&
				yb && (lastPos.y > p2.y || lastPos.y < p1.y) || !yb && (lastPos.y > p1.y || lastPos.y < p2.y)
				&&
				zb && (lastPos.z > p2.z || lastPos.z < p1.z) || !zb && (lastPos.z > p1.z || lastPos.z < p2.z))
			{
				newPos = p2;
				...
				for (int j = 0; j < (int)ribs.size(); ++j) {
					if ((ribs[j])[0] == index1 && (ribs[j])[1] == index2
						|| (ribs[j])[1] == index1 && (ribs[j])[0] == index2) {
						(ribs[j])[2] -= color;
					}
				}
				...
				if (bpath.size() < 2) {
					...
					while (bpath[0] == endVertex) endVertex = rand() % points.size();
					setPath(path(bpath[0], endVertex));
					painted(bpath, color);
				}
			}
			else {
				newPos = NxVec3(lastPos.x + cos(alpha)*objectVelocity*deltaTime,
					lastPos.y + cos(betta)*objectVelocity*deltaTime,
					lastPos.z + cos(gamma)*objectVelocity*deltaTime);
			}
			for (int i = 0; i < (bpath.size() - 2); i++) {
				DrawArrow(points[bpath[i + 1]], points[bpath[i]], getColor(color));
			}
...
void SetupCamera()
{
	...
	glLoadIdentity();
	gluPerspective(65.0f, gCameraAspectRatio, 1.0f, 10000.0f);
...
void InitGlut(int argc, char** argv)
{
	...
	gMainHandle = glutCreateWindow("Gusarov A.A. Lab 3-5");
...
void InitNx()
{
	std::ofstream fout("F:\\Users\\arch\\Documents\\graph.csv", std::ofstream::app);
	...
	objectsCount = min(objectsCount, 37);
	...
	for (int i = 0; i < objectsCount; ++i) {
        ...
        while (startVertex == endVertex) 
            endVertex = rand() % points.size();
		
        Ball temp = Ball(CreateSphere(startVertex));

		// Coordinate calculation loop
		int x, y, z;
...
NxVec3 getColor(int _color) {
	return NxVec3(getColorLayer(_color), getColorLayer(_color), getColorLayer(_color));
}
...
void GenerateGraph(std::vector<NxVec3>& points, std::vector< std::vector<int> >& ribs, int graphPointsCountX, int graphPointsCountY) {
	...
	for (int i = 0; i < graphPointsCountX; ++i) {
		for (int j = 0; j < graphPointsCountY; ++j) {
			...
			points.push_back(NxVec3(1.0f * j + randX, offsetZ * 2 + randZ, 1.0f * i + randY));
		}
	}
...
void GetGraphMatrix(std::vector<NxVec3>& points, std::vector< std::vector<int> >& ribs) {
	...
	for (int i = 0; i < (int)ribs.size(); ++i) {
		...
		(matrix[p1])[p2] = sqrt((x2 - x1)*(x2 - x1) + (y2 - y1)*(y2 - y1) + (z2 - z1)*(z2 - z1));
		(matrix[p2])[p1] = sqrt((x2 - x1)*(x2 - x1) + (y2 - y1)*(y2 - y1) + (z2 - z1)*(z2 - z1));
	}
}
	...
	for (int i = path.size() - 2; i >= 0; --i) {
		for (int j = 0; j < (int)ribs.size(); ++j) {
			...
			if ((ribs[j])[0] == path[i] && (ribs[j])[1] == path[i + 1]
				...
				(ribs[j])[2] = (ribs[j])[2] == 0 ? color : (ribs[j])[2] + color;
			}
		}
	}
}
