...
int rubin(XEvent *ev)
{
    ...
    XGrabPointer(dpy, win, False, (ButtonReleaseMask | ButtonMotionMask), GrabModeAsync, GrabModeAsync, win, None, CurrentTime);
    ...
    else
    {
        ...
        if (abs(ev->xbutton.x - (frag + i)->rec->x) < 10)
            x = (frag + i)->rec->x + (frag + i)->rec->width;
        else
            x = (frag + i)->rec->x;

        if (abs(ev->xbutton.y - (frag + i)->rec->y) < 10)
            y = (frag + i)->rec->y + (frag + i)->rec->height;
        else
            y = (frag + i)->rec->y;

        frag0(ftmp, x, y); /* set reform base point */
        ...
        motion_event.xmotion.x = ev->xbutton.x;
        motion_event.xmotion.y = ev->xbutton.y;
        XSendEvent(dpy, win, False, ButtonMotionMask, &motion_event);
    }
