...
{
    XFig *f = REFIG(ff);
    int a = (f->width) / 2;  /* rect 1/2 x-axe */
    int b = (f->height) / 2; /* rect 1/2 y-axe */
    XPoint p[2];             /* focuses pos */

    if ((x < (f->x - 1)) || (x > (f->x + f->width + 1)) || (y < (f->y - 1)) || (y > (f->y + f->height + 1)))
...
int fragover(XFragment *ff1, XFragment *ff2)
{
    ...
    if ((s1.x1 > s2.x2) || (s2.x1 > s1.x2) || (s1.y1 > s2.y2) || (s2.y1 > s1.y2))
...
{                               /* set extra method number */
    return ((*fe[MINIFRAG])()); /* MINI MAXI MEAN DIFF IDEN GRP2 */
} /* fragextra */
