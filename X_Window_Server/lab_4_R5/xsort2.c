...
int resmng(int argc, char *argv[])
{
    ...
    static char *rname[] = {/* resource spec names */ "xsort.foreground", /* -fg */ "xsort.rubbground", /* -rg */ "xsort.extrground", /* -eg */ "xsort.galtground", /* -ag */ "xsort.background" /* -bg */};                                                                                                                                                               /* rname */
    static char *rdef[] = {/* defaults color resource */ "black", /* foreground all fragments */ "grey", /* rubber fragment color */ "black", /* extra fragment color */ "black", /* group altforeground fragment color */ "white" /* window background */};                                                                                                               /* rdef in rname order */
    static XrmOptionDescRec rtab[] = {/* Resource Command Options */ {"-fg ", ".foreground", XrmoptionSepArg, NULL}, {"-rg ", ".rubbground", XrmoptionSepArg, NULL}, {"-eg ", ".extrground", XrmoptionSepArg, NULL}, {"-ag ", ".galtground", XrmoptionSepArg, NULL}, {"-bg ", ".background", XrmoptionSepArg, NULL}, {"-xrm ", NULL, XrmoptionResArg, NULL}}; /* rtab[] */ /* Check display defaults */

    dpy = XOpenDisplay(NULL);
    scr = DefaultScreen(dpy);
    cmap = DefaultColormap(dpy, scr);
    root = DefaultRootWindow(dpy); /* Load resourses data base */
    XrmInitialize();
    rdb = XrmGetFileDatabase("lab4_res");                                         /* rdb <- lab4_res */
    XrmParseCommand(&rdb, rtab, NGC + 1, "xsort", &argc, argv); /* argv -> rdb */ /* Extract resourses from data base to GC */
    puts("\nColor resources:");

    for (i = 0; i < NGC; i++)
    {
        if (XrmGetResource(rdb, rname[i], NULL, &rtype, rval) == False)
            rval->addr = rdef[i]; /* set default for missing resource */ /* or append resource to rdb from default */ /* while(XrmGetResource(rdb, rname[i], NULL, &rtype, rval) == False) */ /* XrmPutStringResource(&rdb, rname[i], rdef[i]); */ /* or rval->addr = rdef[i]; rval->size = strlen(rdef[i]); */ /* XrmPutResource(&rdb, rname[i], "String", rval); */
    ...
    if (rdb != NULL)             /* Free RDB space */
        XrmDestroyDatabase(rdb); /* Correct background context */
...
int canvas()
{
    ...
    attr.bit_gravity = NorthWestGravity; /* reconfig Anti-blink */ /* or attr.bit_gravity = StaticGravity; */
    ...
    XStoreName(dpy, win, "R5"); /* Window title */
    emask = (ButtonPressMask | ButtonReleaseMask | ButtonMotionMask | ExposureMask | KeyPressMask);
